import { getDb, rowsToObjects, getWaNotifyAsk, setWaNotifyMode, clearWaNotifyAsk, getSystemSetting } from './db.js';
import { getAiConfig, openaiChat, openaiReadImage, openaiReadDocument, openaiTranscribeAudio } from './ai.js';

// Modelo de chat/texto da OpenAI para TODA a interação do WhatsApp (classificação,
// extração de itens, respostas). Groq/Gemini não são usados aqui.
function _chatModel(cfg) { return cfg.openaiVisionModel || 'gpt-4o-mini'; }
...
// Lê o conteúdo de uma imagem/print via ChatGPT (OpenAI) — único provedor de visão.
async function readImageContent(cfg, base64, mime) {
  if (!cfg.openaiKey) { const e = new Error('no vision provider'); e.kind = 'tech'; throw e; }
  let t = '';
  try {
    t = await openaiReadImage({ key: cfg.openaiKey, model: cfg.openaiVisionModel, base64, mime, prompt: MEDIA_EXTRACT_PROMPT });
  } catch (e) {
    console.error('[assistant] leitura de imagem (openai) falhou', e?.message);
    const err = new Error('image read failed'); err.kind = 'tech'; throw err;
  }
  if (t && t.trim()) return t.trim();
  const err = new Error('image read empty'); err.kind = 'empty'; throw err;
}

// Lê o conteúdo de um documento (PDF/fatura) via ChatGPT (OpenAI) — único provedor.
async function readDocumentContent(cfg, base64, mime) {
  if (!cfg.openaiKey) { const e = new Error('no doc provider'); e.kind = 'tech'; throw e; }
  let t = '';
  try {
    t = await openaiReadDocument({ key: cfg.openaiKey, model: cfg.openaiVisionModel, base64, mime, prompt: MEDIA_EXTRACT_PROMPT });
  } catch (e) {
    console.error('[assistant] leitura de documento (openai) falhou', e?.message);
...
async function answerHelpQuestion(cfg, user, question) {
  if (!cfg.openaiKey) return HELP_OVERVIEW;
  try {
    const composed = await openaiChat({
      key: cfg.openaiKey,
      model: _chatModel(cfg),
      messages: [
...
Responda APENAS sobre o funcionamento do sistema. Se perguntarem algo fora disso, oriente gentilmente a usar o assistente para finanças.` },
    ...
    console.error('[assistant] ajuda (openai) falhou', e?.message);
...
// Regras de uso PADRÃO do assistente (baked-in). Sempre injetadas no prompt.
// O administrador pode ESTENDER/AJUSTAR essas regras pelo painel (setting ai_rules)
// e adicionar documentos na Base de conhecimento (knowledge_docs) — ambos entram
// no prompt via getGovernance(). Isso mantém tudo editável pela administração sem
// depender de escrita no banco: há sempre um baseline sólido de comportamento.
const DEFAULT_USAGE_RULES = `1. Entenda a INTENÇÃO antes de agir; só pergunte o que for essencial e faça o resto por conta própria.
2. Respeite SEMPRE o nível de acesso: usuário comum enxerga apenas os próprios dados; nunca exponha dados de terceiros.
3. Valores em Real (R$). Datas relativas ("hoje", "ontem", "sexta") viram data real no fuso do Brasil.
4. Ao registrar, confirme de forma curta e humana; jamais invente dados que a pessoa não informou.
5. Em dúvida real entre entrada e saída de dinheiro, use "clarify" com uma pergunta leve.
6. Gestão de usuários e mensagens em massa (broadcast) são exclusivas de administradores.
7. Todos os menus, cadastros e fluxos já existentes continuam valendo — apenas responda de forma mais inteligente e natural.
8. Seja breve, caloroso e natural; no máximo 1 emoji por mensagem.`;

// Carrega a governança editável pela administração: regras (setting ai_rules) +
// documentos ativos da Base de conhecimento (knowledge_docs). Tolerante a falha.
async function getGovernance() {
  let rules = '';
  let docs = [];
  try {
    rules = (await getSystemSetting('ai_rules').catch(() => '')) || '';
  } catch { rules = ''; }
  try {
    const { rows } = await getDb().execute(
      'SELECT title, content, type, tags FROM knowledge_docs WHERE enabled=1 ORDER BY type ASC, updated_at DESC'
    );
    docs = rowsToObjects(rows);
  } catch { docs = []; }
  return { rules: String(rules).trim(), docs };
}

function buildSystemPrompt(user, accounts = [], governance = { rules: '', docs: [] }) {
  const isAdmin = user.is_admin === 1 || user.role === 'admin' || user.role === 'super_admin';
  const accessLabel = user.role === 'super_admin'
    ? 'ADMINISTRADOR MASTER (acesso total ao sistema e a todos os usuários)'
    : isAdmin
    ? 'ADMINISTRADOR (consulta dados de todos os usuários e gerencia cadastros)'
    : 'USUÁRIO COMUM (só acessa a própria conta)';
  const accList = accounts.length
    ? accounts.map((a) => `- ${a.name}${a.bank_name ? ` (${a.bank_name})` : ''}`).join('\n')
    : '- (nenhuma conta/carteira cadastrada ainda)';

  const adminRules = (governance.rules || '').trim();
  const rulesBlock = `${DEFAULT_USAGE_RULES}${adminRules ? `\n\nREGRAS ADICIONAIS DEFINIDAS PELA ADMINISTRAÇÃO (têm prioridade):\n${adminRules}` : ''}`;
  const docsBlock = (governance.docs && governance.docs.length)
    ? governance.docs.map((d) => `• ${d.title}${d.type ? ` [${d.type}]` : ''}: ${String(d.content || '').trim()}`).join('\n')
...
O QUE O SISTEMA FAZ (visão geral — use para entender a demanda e orientar a pessoa):
- Lançamentos de receitas e despesas (fixas ou variáveis), à vista ou parceladas.
- Contas, carteiras e cartões (com dia de fechamento/vencimento).
- Categorias de receita e despesa personalizadas.
- Contas a pagar: cadastro, lembrete diário e baixa (marcar como pago).
- Metas financeiras e resumos do mês.
- Leitura de faturas e comprovantes por texto, áudio ou imagem.${isAdmin ? '\n- (Admin) Gestão de usuários do sistema e mensagens em massa (broadcast).' : ''}

CONTAS/CARTEIRAS JÁ CADASTRADAS DO USUÁRIO:
${accList}

REGRAS DE USO (definidas pela administração — SIGA-AS À RISCA):
${rulesBlock}

BASE DE CONHECIMENTO (consulte para responder dúvidas de uso do sistema):
${docsBlock}
...
export async function handleAssistantMessage(msg, instanceName) {
  ...
  try {
    if (m.conversation) {
      ...
      // Áudio: transcrição via ChatGPT (OpenAI Whisper) — único provedor.
      if (!cfg.openaiKey) { await reply('Recebi seu áudio, mas a interpretação de áudio ainda não está configurada. Pode me mandar por texto? 🙂'); return { handled: true, reason: 'no_audio_provider' }; }
      const b64 = await getMediaBase64(inst.name, inst.api_key, msg.key);
      const mime = m.audioMessage.mimetype || 'audio/ogg';
      if (b64) {
        try {
          userText = await openaiTranscribeAudio({ key: cfg.openaiKey, model: cfg.openaiAudioModel, base64: b64, mime });
        } catch (e) {
          console.error('[assistant] transcrição de áudio (openai) falhou', e?.message);
        }
      }
    } else if (m.imageMessage) {
      userText = m.imageMessage.caption || '';
      // Interpretação de imagem a cargo do ChatGPT (OpenAI).
      if (!cfg.openaiKey) {
      ...
      // Documentos (PDF/fatura): leitura via ChatGPT (OpenAI).
      const docMsg = m.documentMessage || m.documentWithCaptionMessage?.message?.documentMessage || {};
      userText = docMsg.caption || '';
      if (!cfg.openaiKey) {
        await reply('A *leitura de documentos (PDF)* precisa da OpenAI configurada. Me envie os dados por texto que eu registro (ex.: "comprei uma TV por 3500 em 8x"). 🙂');
  ...
  if (!cfg.openaiKey) {
    ...
    return { handled: true, reason: 'no_openai' };
  }
  ...
  try {
    const raw = await openaiChat({
      ...
      model: _chatModel(cfg),
      jsonMode: true,
      temperature: 0.4, // um pouco de calor p/ respostas menos robóticas (JSON segue válido)
      messages: [
        { role: 'system', content: buildSystemPrompt(user, accounts, governance) },
    ...
    console.error('[assistant] classificador (openai) falhou', e?.message);
    await reply('Não consegui pensar na resposta agora. Pode tentar de novo em instantes? 🙏');
    return { handled: true, reason: 'openai_error' };
  }
